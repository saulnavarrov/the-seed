module.exports = {
  ...
  friendlyName: 'Checking password attempt',
  ...
  fn: function(inputs, exits) {
    ...
    bcrypt.compare(inputs.passwordAttempt, inputs.hashedPassword, (err, ok) => {
      ...
      if (!ok) {
        sails.log.verbose('Password attempt did not match already-encrypted version.')
        return exits.success({
          error: null,
          success: 'INCORRECT',
          message: 'Password attempt did not match already-encrypted version.'
        });
      }
      ...
      return exits.success({
        error: null,
        success: 'OK',
        message: 'Password attempt matched the already-encrypted version.'
      });
