      ...
      before: {
        '/*': {
          ...
          fn: async function (req, res, next) {
            ...
            // Por si el usuario viene cargado en la body de una
            // request
            ...
            // Recargando datos del usuario en la this.req
            req.me = loggedInUser;

            // Eliminando contraseña de sesion
            delete req.me.password;

            // Eliminando sesion afectada si viene de una consuta
            // dentro del body
            if(!_.isUndefined(req.body)){
              if (!_.isUndefined(req.body.userId)) {
                delete req.session.userId;
            ...
            if (req.method === 'GET') {
              ...
              for (let attrName in TxUsers.attributes) {
                if (TxUsers.attributes[attrName].protect) {
